...
public class ABTree {
	...
	/**
	 * Diese Klasse repräsentiert einen Knoten des Baumes. Es kann sich dabei um
	 * einen inneren Knoten oder ein Blatt handeln.
	 */
	...
	private class ABTreeInnerNode extends ABTreeNode {
		...
		public void insert(int key) {
			for (int i = 0; i < children.size(); i++) {
				// Fehlerbehandlung
				if (key == keys.get(i)) {
					throw new RuntimeException("Key gibt es schon");
				}
				// Wenn Blatt
				if (children.get(i).height() == 0) {
					children.set(i, new ABTreeInnerNode(key));
				}
				// Tiefere Suche
				if (key < keys.get(i)) {
					children.get(i).insert(key);
				}
				// Wenn größer als alle anderen Elemente => suche weiter rechts
				if (key > keys.get(i) && i == children.size() - 1) {
					children.get(i).insert(key);
				}
			}
		}
		...
		public int height() {
			for (int i = 0; i < children.size();) {
				...
				return children.get(i).height() + 1;
	...
	private class ABTreeLeaf extends ABTreeNode {
		@Override
		public void insert(int key) {
			throw new RuntimeException("Ein Blatt kann keinen Wert besitzen");
		}
